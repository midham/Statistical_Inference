...
#Title: Impact of different vitamin C dose levels and supplement types on tooth lengths of Guinea pig
##Author: midham76
...
This experiment will analyze the `TootGrowth` data of the R data sets package which describes the effect of vitamin C on tooth growth in guinea pigs
...
Dose will be treated  as a factor in the subsequent analysis. Only three dose levels of Vitamin C (0.5, 1, and  2 mg) were used.

A total of 60 unique guinea pigs will be used. Each guinea pig was assigned to a group which received a specific dose level of vitamin C. In addition, in each of those groups two different delivery methods (orange juice or ascorbic acid) were applied leaving 10 pigs per subgroup:
...
However, the experiment is interested in finding differences in tooth length across different groups of supplement types, dose levels and their respective combinations.
...
The left box plot above depict that guinea pigs which received their dose of vitamin C using orange juice have on average a greater tooth length than those pigs which got their dose using ascorbic acid. The orange juice group distribution is skewed to the left whereas the ascorbic acid group seems to be fairly symmetric. 
...
As the experiment is dealing with subsets of n <= 30, standard error estimate will generally not be accurate. Therefore, experiment will use the t distribution for our hypothesis tests and when constructing confidence intervals.
...
However, the experiment assumed that the guinea pigs were randomly assigned to one of the groups (guarantees independence) and that they were sampled from a nearly normal population.

### Differences in supplement types

The experiment will check if the observed difference in tooth length means between pigs who received their dose using orange juice and those who received their dose via ascorbic acid is statistically different from 0.

The conservative choice will be a t-test with unequal variances of the two samples. The table below shows the results:
...
With a p-value of `r round(t_diff_supp$p.value, 3)` which is larger than the significance value, 0.05, the experiment failed to reject the null hypothesis. The data does not provide strong evidence that the average difference in tooth length across the two supplement types is different from 0. 
...
Since the experiment are dealing with 3 different dose levels we need to apply 3 t-tests to cover all factor level combinations. Again, our null hypothesis in all cases is that the average difference in tooth length  is 0.
...
The experiment is 95% confident that the average tooth length of guinea pigs who received a 2mg dose of vitamin C is on average `r round(pairwise_results$conf.low[1], 2)` to `r round(pairwise_results$conf.high[1], 2)` millimeters higher than those who received a 1mg dose.


# Conclusions

The experiment was able to show that the observed difference of tooth length mean across supplement types is statistically not different from 0.

The experiment also highlight the fact that tooth length indeed varies across groups of dose levels and that all of those differences are statistical and pramatically significant. 
